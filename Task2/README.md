...
- `requests.cpu` снижен `100m → 50m`: idle ≈ 20m = 40% (ниже 80%, без лишнего масштабирования на
  простое), а под реальной нагрузкой CPU превышает 40m (80%) ⇒ HPA масштабирует поды `1 → N`.
